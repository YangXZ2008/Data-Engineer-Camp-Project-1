...
class FuelAPIClient:
    ...
    def get_access_token(self):
        if os.path.exists(self.access_token_file):
            ...
            issued_time = datetime.fromtimestamp(
                int(token_info["issued_at"])/1e3)
        ...
        response = requests.request(
            "GET", self.ACCESS_TOKEN_URL, headers=headers, params=query_string)
        if response.status_code != 200:
            raise Exception(
                f"Failed to extract data from the Fuel API. Status Code: {response.status_code}. Response: {response.text}")
